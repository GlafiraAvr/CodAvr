unit RepSvodVedDModulePer;
 //{$DEFINE FROMSNAPSHOT}
interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, RepSvodVedDModule, DB, FR_DSet, FR_DBSet, IBCustomDataSet,
  IBDatabase, RxMemDS, FR_Class, HelpClasses, FR_ChBox, FR_Desgn, SnapShotManagerDModule,
  xmldom, XMLIntf, msxmldom, XMLDoc;

type
  Tdm_RepSvodVedPer = class(Tdm_RepSvodVed)
...
var
  dm_RepSvodVedPer: Tdm_RepSvodVedPer;
...
function Tdm_RepSvodVedPer.PrepareDsets: boolean;
var _cdate: TDate;
    _list: TStringList;
    _ind :integer;
begin
...
procedure Tdm_RepSvodVedPer.MyFillSnapList(var list: TStringList);
var
  _sql : string;
  _sItem : string;
begin
  ...
  try
   list.Clear;
    _sql :='select id, dt from SNSHOT_SNAPSHOTS where (id_type=1) and (dt>= '''+ DateToStr(F_DtBeg) +
...
procedure Tdm_RepSvodVedPer.FrMemoManage;
...
procedure Tdm_RepSvodVedPer.SnapSum(ds_From, ds_To: TDataSet);
...
procedure Tdm_RepSvodVedPer.CalcVsego;
...
procedure Tdm_RepSvodVedPer.InitCountByReg(ds_Init : TDataSet);
var
  reg_cond: string;
begin
...
{  'select sf.field_name ,sv.row_number, sv.field_value from snshot_snapshots ss
'join snshot_dsets sd on ((ss.id = sd.id_snapshot) and (ss.dt = '04.11.2008'))'
'left join snshot_fields sf on (sf.id_dset = sd.id)                           '
'left join snshot_values sv on ((sv.id_field = sf.id) and (sv.row_number<10))'}
