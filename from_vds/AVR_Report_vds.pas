...
type
  ...
  TOpenOrderSearch_vds=class(TBaseReport)
  private
  ...
  TCloseOrderSearch_vds=class(TBaseReport)
  private
    function DateCaption(dt1, dt2: string): string;
  protected
    procedure InitFields; override;
  ...
  TAllOrderSearch_vds=class(TBaseReport)
  private
...
uses HelpFunctions, AnalysisViewForm, PeriodRegDmgSelForm,
    OrderForm_vds, ExcelWorksForm, StringConsts;
...
begin
  ...
  begin
    ...
    frm_Order:=Tfrm_Order_vds.Create(nil, OrderID);
    frm_Order.ShowModal;
  end;
end;
...
procedure TOpenOrderSearch_vds.CreateForms;
begin
  F_OptionForm:=Tfrm_PeriodRegDmgSel.Create(nil, 'Поиск незакрытых нарядов ВДС');
 (F_OptionForm as Tfrm_PeriodRegDmgSel).btn_DamagePlace.Visible:=false; 
  F_ResultForm:=Tfrm_AnalysisView.Create(nil, 'Просмотр незакрытых нарядов ВДС');
end;

function TOpenOrderSearch_vds.DateCaption(dt1, dt2: string): string;
begin
...
procedure TOpenOrderSearch_vds.InitFields;
begin
  F_Name:='Поиск незакрытых нарядов за период';
  F_ReportFileName:='from_vds\OpenOrderSearch.frf';
end;

procedure TOpenOrderSearch_vds.PrepareDsets;
var
...
begin
 //(F_OptionForm as Tfrm_PeriodRegDmgSel).btn_okClick(nil);
  ...
  CountSQL:='select count(id) RecordCount from orders_vds  o '+
            (F_OptionForm as Tfrm_PeriodRegDmgSel).ResultCondition+
            ' and IsClosed=0';
...
procedure TOpenOrderSearch_vds.PrepareFastReport;
var i,j:integer;
begin
...
procedure TOpenOrderSearch_vds.PrepareResultFormBtns;
begin
...
procedure TOpenOrderSearch_vds.PrepareResultFormGrid;
begin
  F_GridViewBuilder.Free;
...
procedure TOpenOrderSearch_vds.PrepareResultFormHeader;
begin
...
procedure TCloseOrderSearch_vds.CreateForms;
begin
  F_OptionForm:=Tfrm_PeriodRegDmgSel.Create(nil, 'Поиск закрытых нарядов ВДс');
  (F_OptionForm as Tfrm_PeriodRegDmgSel).btn_DamagePlace.Visible:=false;
  F_ResultForm:=Tfrm_AnalysisView.Create(nil, 'Просмотр закрытых нарядов ВДС');
end;

function TCloseOrderSearch_vds.DateCaption(dt1, dt2: string): string;
begin
   if dt1<>'' then dt1:=  DateToStr(StrToDateTime(dt1));
   if dt2<>'' then dt2:=  DateToStr(StrToDateTime(dt2));
...
procedure TCloseOrderSearch_vds.InitFields;
begin
  F_Name:='Поиск закрытых нарядов за период';
  F_ReportFileName:='from_vds\ClosedOrderSearch.frf';
end;

procedure TCloseOrderSearch_vds.PrepareDsets;
var
  SelSQL, CountSQL: string;
begin
...
procedure TCloseOrderSearch_vds.PrepareFastReport;
var i,j:integer;
begin
  with (F_OptionForm as Tfrm_PeriodRegDmgSel) do
  begin
...
procedure TCloseOrderSearch_vds.PrepareResultFormBtns;
begin
  with F_ResultForm as Tfrm_AnalysisView do
  begin
    btn_Order.OnClick:=OrderBtnClick;
...
procedure TCloseOrderSearch_vds.PrepareResultFormGrid;
begin
  F_GridViewBuilder.Free;
  F_GridViewBuilder:=TGridViewBuilder.Create(F_Dset[0],
                  (F_ResultForm as Tfrm_AnalysisView).Grid);
...
procedure TCloseOrderSearch_vds.PrepareResultFormHeader;
begin
...
procedure TAllOrderSearch_vds.BoltStatusCellShow(ShowValue: variant;
  var Res: string);
var
  val:integer;
begin
...
procedure TAllOrderSearch_vds.CreateForms;
begin
  F_OptionForm:=Tfrm_PeriodRegDmgSel.Create(nil, 'Поиск всех нарядов ВДС');
 (F_OptionForm as Tfrm_PeriodRegDmgSel).btn_DamagePlace.Visible:=false;
  F_ResultForm:=Tfrm_AnalysisView.Create(nil, 'Поиск всех нарядов ВДС');
end;

function TAllOrderSearch_vds.DateCaption(dt1, dt2: string): string;
begin
  if dt1<>''then dt1:=  DateToStr(StrToDateTime(dt1));
 if dt2<>''then  dt2:=  DateToStr(StrToDateTime(dt2));
...
procedure TAllOrderSearch_vds.InitFields;
begin
  F_Name:='Поиск всех нарядов ВДС за период';
  F_ReportFileName:='from_vds\RepAllOrders.frf';
end;

procedure TAllOrderSearch_vds.IsClosedGroupHeaderCaption(DataSet: TDataSet;
  AggregateValueArr: array of double; var OutRow: TStringList);
begin
  with OutRow do
  begin
...
procedure TAllOrderSearch_vds.PrepareDsets;
var
  SelSQL, CountSQL: string;
begin
  SelSQL :=
...
procedure TAllOrderSearch_vds.PrepareFastReport;
var i,j:integer; 
begin
  with (F_OptionForm as Tfrm_PeriodRegDmgSel) do
  begin
...
procedure TAllOrderSearch_vds.PrepareResultFormBtns;
begin
  with (F_ResultForm as Tfrm_AnalysisView) do
  begin
    btn_Order.OnClick:=OrderBtnClick;
...
procedure TAllOrderSearch_vds.PrepareResultFormGrid;
var
  group: TGroup;
begin
  F_GridViewBuilder.Free;
...
procedure TAllOrderSearch_vds.PrepareResultFormHeader;
begin
  with (F_OptionForm as Tfrm_PeriodRegDmgSel),
        (F_ResultForm as Tfrm_AnalysisView) do
  begin
