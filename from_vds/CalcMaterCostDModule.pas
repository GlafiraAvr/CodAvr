...
type
  Tdm_CalcMaterCost = class(Tdm_NGReportBase)
...
function Tdm_CalcMaterCost.FillResultDset: boolean;
var
  RowNumber: integer;
begin
  ResultDset.Close;
...
procedure Tdm_CalcMaterCost.Init;
begin
  SetSqlString;
  SetRecordCount;
end;

function Tdm_CalcMaterCost.PrepareDset: boolean;
begin
  Result:=false;
...
begin
  ...
  SqlBottom:=Format(
    ' from Departures d'+
    ' left join Orders o on o.id=d.fk_departures_orders'+
    ' where'+
    ' exists(select dm.id from departurematerials dm where dm.fk_usedmaterials_departure=d.id)'+
    ' %s %s', [DtCond, RegCond]);
...
procedure Tdm_CalcMaterCost.ResultDsetCalcFields(DataSet: TDataSet);
begin
  inherited;
...
function Tdm_CalcMaterCost.GetWorks(DepID: integer): string;
begin
  Result:='';
