unit SnapShotManagerDModule_vds;
...
begin
  OpenDset('select id from snshot_types_vds where name='+QuotedStr(F_SnapShotType));
...
begin
  ExecSQL(Format('insert into snshot_types_vds(name) values(''%s'')',[F_SnapShotType]));
end;

function Tdm_SnapShotManager.SelShotID(id_type: integer): integer;
begin
  OpenDset(Format('select id, cur_user from snshot_snapshots_vds s where (id_type=%d) and (dt=''%s'')',
...
begin
  ExecSQL(Format('delete from snshot_snapshots_vds where id=%d',[id_short]));

  ExecSQL(Format(
    ' delete from snshot_values_vds where id_field in'+
    '   (select id from snshot_fields_vds where id_dset in'+
    '     (select id from snshot_dsets_vds where id_snapshot=%d))',
    [id_short]));

  ExecSQL(Format(
    ' delete from snshot_fields_vds where id_dset in'+
    ' (select id from snshot_dsets_vds where id_snapshot=%d)',
    [id_short]));

  ExecSQL(Format('delete from snshot_dsets_vds where id_snapshot=%d',
    [id_short]));
end;

procedure Tdm_SnapShotManager.InsShort(id_type: integer);
begin
  ExecSQL(Format('insert into snshot_snapshots_vds(dt, id_type, cur_user) values(''%s'', %d , ''' +
...
begin
  ExecSQL(Format(' insert into snshot_dsets_vds(name, id_snapshot)'+
                ' values(''%s'', %d)',[name, id_shot]));
end;

function Tdm_SnapShotManager.SelMaxDsetID(id_shot: integer): integer;
begin
  OpenDset('select max(id) id from snshot_dsets_vds where id_snapshot='+IntToStr(id_shot));
  Result:=dset.FieldByName('id').AsInteger;
end;

procedure Tdm_SnapShotManager.InsField(id_dset: integer; name: string);
begin
  ExecSQL(Format( ' insert into snshot_fields_vds(id_dset, field_name)'+
                  ' values(%d, ''%s'')',
                  [id_dset, trim(name)]));
end;
...
begin
  OpenDset('select max(id) id from snshot_fields_vds where id_dset='+IntToStr(id_dset));
  Result:=dset.FieldByName('id').AsInteger;
end;
...
begin
  ...
  ExecSQL(Format( ' insert into snshot_values_vds(id_field, field_value, row_number)'+
                  ' values(%d, %s, %d)',
...
begin
  ...
  TypeID:=SelSNShotTypeID;
  if TypeID=-1 then
    raise Exception.Create('В таблице SNSHOT_TYPES_vds отсутствует значение равное полю SnapShotType');
  {/Находим TypeID}
...
begin
  ...
  try
    ...
    list.Clear;
    OpenDset('select id, dt from SNSHOT_SNAPSHOTS_vds where id_type='+IntToStr(TypeID)+' order by dt desc');
