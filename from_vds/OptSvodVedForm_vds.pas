...
type
  Tfrm_OptSvodVed_vds = class(Tfrm_BaseOption)
    btn_ok: TBitBtn;
    btn_exit: TBitBtn;
    de_Date: TDateEdit;
    Label1: TLabel;
...
procedure Tfrm_OptSvodVed_vds.btn_okClick(Sender: TObject);
begin
  inherited;
...
procedure Tfrm_OptSvodVed_vds.btn_exitClick(Sender: TObject);
begin
  Close;
end;

procedure Tfrm_OptSvodVed_vds.FormShow(Sender: TObject);
begin
  inherited;
  F_strRegions:='Все';
  F_RegionsID:='';
...
procedure Tfrm_OptSvodVed_vds.FormCreate(Sender: TObject);
begin
  inherited;
  if (Date>2) then
  de_Date.Date:=Date-1;
