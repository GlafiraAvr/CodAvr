...
type
  Tdm_RepOpenOrders_vds = class(Tdm_NGReportBase)
...
function Tdm_RepOpenOrders_vds.FillResultDset: boolean;
...
function Tdm_RepOpenOrders_vds.GetWorks(OrderID: integer): string;
begin
  Result:='';
...
procedure Tdm_RepOpenOrders_vds.Init;
begin
  SetSqlString;
  SetRecordCount;
end;

function Tdm_RepOpenOrders_vds.PrepareDset: boolean;
begin
  Result:=false;
...
//    ' left join disconnections d on o.id=d.fk_discon_order'+
  ...
  F_MainSqlString:=
    ...
    ' , (select messagename from s_messagetypes_vds where id=o.fk_orders_messagetypes) MT'+
