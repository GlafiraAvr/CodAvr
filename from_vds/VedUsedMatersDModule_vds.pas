...
type
  Tdm_VedUsedMaters_vds = class(Tdm_NGReportBase)
...
function Tdm_VedUsedMaters_vds.FillResultDset: boolean;
var
  i: integer;
begin
  ResultDset.Close;
...
function Tdm_VedUsedMaters_vds.GetMaters(OrderID: integer): string;
begin
  Result:='';
...
procedure Tdm_VedUsedMaters_vds.Init;
begin
  SetSqlString;
  SetRecordCount;
end;

function Tdm_VedUsedMaters_vds.PrepareDset: boolean;
begin
  Result:=false;
...
begin
  ...
  SqlBottom:=Format(
    ' from Orders_vds o'+
    ' where (o.isclosed=1)'+
    ' and exists(select id from departurematerials_vds  where fk_usedmaterials_orders=o.id)'+
    ' %s %s',[DtCond, RegCond]);
...
procedure Tdm_VedUsedMaters_vds.dset_MatersCalcFields(DataSet: TDataSet);
begin
  with DataSet do
  begin
